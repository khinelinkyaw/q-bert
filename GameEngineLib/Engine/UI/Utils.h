...
namespace GameEngine
{
    namespace UI
    {
        ...
        inline vec2 AlignToWindow(float hPercent, float vPercent)
        {
        ...
        inline vec2 AlignToRect(float hOffset, float vOffset, Rect<float> rect, Pivot pivot = Pivot::LeftUp)
        {
            auto [hDir, vDir] { Constants::UI_PIVOT_MAP.at(pivot) };
        ...
        inline vec2 AlignToRect(vec2 offset, Rect<float> rect, Pivot pivot = Pivot::LeftUp)
        {
            auto [hDir, vDir] { Constants::UI_PIVOT_MAP.at(pivot) };
