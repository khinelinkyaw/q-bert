...
void Renderer::RenderTexture(const Texture2D& texture, const float x, const float y, double angle) const
{
    SDL_FRect dst{};
    dst.x = x;
    dst.y = y;
    SDL_GetTextureSize(texture.GetSDLTexture(), &dst.w, &dst.h);
    RenderTexture(texture, nullptr, &dst, angle);
}

void Renderer::RenderTexture(const Texture2D& texture, const float x, const float y, const float width, const float height, double angle) const
{
    SDL_FRect dst{};
    dst.x = x;
    dst.y = y;
...
void GameEngine::Renderer::RenderTexture(const Texture2D& texture, SDL_FRect const* pSrcRect, SDL_FRect const* pDstRect, double angle) const
{
    SDL_RenderTextureRotated(GetSDLRenderer(), texture.GetSDLTexture(), pSrcRect, pDstRect, angle, nullptr, SDL_FLIP_NONE);
}
